...
const SETUP =
  "Desk setup required: install pi 0.85.1 and pi-mcp-adapter 2.32.1, and configure gpt-6-astra with max reasoning in the existing Pi client. No fallback was selected."
...
function packageRoot(entry, name, version) {
  ...
  while (true) {
    try {
      ...
      if (error.message === SETUP) throw error
    }
...
export function rpcRefusal(command, provider) {
  if (
    ...
    return "Invalid pi RPC input"
  if (
    command.type === "set_model" &&
    (command.modelId !== "gpt-6-astra" || command.provider !== provider)
  )
    return "Desk requires its configured gpt-6-astra provider; relaunch to change it"
  if (command.type === "set_thinking_level" && command.level !== "max")
    return "Desk requires max reasoning; reasoning change refused"
  if (
    [
      "cycle_model",
    ...
    return "Desk model and session are bound to this launch; start a new task run to change them"
  return undefined
}
...
function main() {
    ...
    const providers =
      result.status === 0
        ? (result.stdout ?? "")
            ...
            .filter((cells) => cells[1] === "gpt-6-astra")
            .map((cells) => cells[0])
        : []
    process.stdout.write(
      JSON.stringify({ adapter, modelAvailable: providers.length > 0 }) + "\n"
    )
    return
  }
  if (!process.env.CODEG_DESK_TOKEN || !process.env.CODEG_DESK_SOCKET)
    throw new Error("Desk bridge is unavailable; launch from a live Desk task")
  const supplied = process.argv.slice(2)
  // pi-acp's immutable launch contract: --mode rpc --no-themes [--session path].
  let session
  for (let i = 0; i < supplied.length; i++) {
    if (supplied[i] === "--mode" && supplied[i + 1] === "rpc") {
      i++
      continue
    }
    if (supplied[i] === "--no-themes") continue
    if (supplied[i] === "--session" && supplied[i + 1]) {
      session = supplied[++i]
      continue
    }
  ...
  createInterface({ input: child.stdout }).on("line", (line) => {
    ...
    if (message.id === probe) {
      ...
      if (
        !message.success ||
        !names.includes("desk-status") ||
        !names.includes("mcp")
      ) {
        fail("Desk extension discovery failed; prompts are blocked")
        return
      }
      child.stdin.write(
        JSON.stringify({ type: "get_state", id: stateProbe }) + "\n"
      )
      return
    }
    if (message.id === stateProbe) {
      if (
        !message.success ||
        message.data?.model?.id !== "gpt-6-astra" ||
        message.data?.thinkingLevel !== "max"
      ) {
        fail(SETUP)
        return
      }
    ...
    .on("line", (line) => {
      ...
      if (ready) forward(line)
        ...
        if (queuedBytes > 1024 * 1024) {
          fail("Desk startup input exceeded its limit")
          return
        }
        queued.push(line)
      }
