...
use crate::db::{
    entities::{ops_agent_rule as rule, ops_agent_scope as scope},
    error::DbError,
};
...
pub(super) async fn gate(
    ...
    let agent = ctx.agent;
    let resource = action
        .resource(payload, ctx)
        .await
        .map_err(|_| DbError::Validation("action resource unavailable".into()))?;
    ...
    let mode = scopes
        .iter()
        .find(|s| resource.is_some() && s.resource == resource)
    ...
    let mut rules = rule::Entity::find()
        ...
        .await?;
    rules.retain(|r| {
        (r.resource.is_none() || r.resource == resource)
            && (r.action_name.is_none() || r.action_name.as_deref() == Some(action.name()))
    });
    rules.sort_by_key(|r| {
        (
            std::cmp::Reverse(r.resource.is_some()),
            std::cmp::Reverse(r.action_name.is_some()),
            r.id,
        )
    });
    let result = |verdict, reason, matched_rule_id| GateResult {
        verdict,
        reason,
        mode: mode.clone(),
        matched_rule_id,
    ...
    if action
        .is_destructive(payload)
        .map_err(|_| DbError::Validation("action safety classification unavailable".into()))?
    {
