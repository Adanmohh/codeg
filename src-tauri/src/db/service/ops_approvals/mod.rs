...
use crate::db::{
    entities::{
        ops_audit_log as audit, ops_proposal as proposal,
...
/// The same transaction and principal used by the gate. Resource/permission
/// lookups must read here, never from a stale preloaded ticket or another pool.
/// Callbacks may read local state; they must not mutate state or perform sends.
pub struct ActionContext<'a> {
    pub db: &'a DatabaseTransaction,
    pub agent: &'a str,
...
pub trait Action: Send + Sync {
    ...
    fn is_destructive(&self, _payload: &Value) -> Result<bool, DbError> {
        Ok(true)
    }
    fn private_fields(&self) -> &'static [&'static str] {
        &[]
    }
    async fn preview(&self, payload: &Value, _ctx: &ActionContext<'_>) -> Result<Value, DbError> {
        Ok(payload.clone())
    }
}
...
pub enum ProposalOutcome {
    Denied(GateResult),
    Pending(Box<proposal::Model>),
    Authorized(AuthorizedAction),
}

fn conflict() -> DbError {
    DbError::Conflict("stale or already resolved approval".into())
}
fn validate(action: &dyn Action, payload: &Value) -> Result<(), DbError> {
    ...
    if !payload.is_object() || redaction::scrub(payload, &[]) != *payload {
        return Err(DbError::Validation(
            "payload must be an object without credentials".into(),
        ));
    }
    action
        .validate(payload)
        .map_err(|_| DbError::Validation("invalid action payload".into()))
}

async fn audit_record<C: ConnectionTrait>(
    conn: &C,
    task_id: i32,
    run_seq: i32,
    actor: &str,
    ...
    audit::ActiveModel {
        ...
        detail: Set(redaction::scrub(&detail, &[]).to_string()),
...
async fn task_cas(
    ...
    let task = work_task::Entity::find_by_id(task_id)
        ...
        .await?
        .ok_or_else(conflict)?;
    if crate::db::entities::folder::Entity::find_by_id(task.folder_id)
...
pub async fn propose(
    ...
    let result = gating::gate(action, &payload, &ctx).await?;
    audit_record(
        &txn,
        task_id,
        run_seq,
    ...
    let preview = action
        .preview(&payload, &ctx)
        .await
        .map_err(|_| DbError::Validation("invalid preview".into()))?;
    ...
    let row = proposal::ActiveModel {
        ...
        created_at: Set(Utc::now()),
        resolved_at: Set(auto.then(Utc::now)),
        ..Default::default()
    }
    .insert(&txn)
    .await?;
    ...
    audit_record(&txn, task_id, run_seq, agent,
        ...
        json!({"proposal_id": row.id, "action": action.name(), "payload_sha256": payload_digest(&payload)})).await?;
    txn.commit().await?;
    Ok(if auto {
        ProposalOutcome::Authorized(AuthorizedAction {
            proposal_id: row.id,
            action_name: row.action_name,
            payload,
        })
    } else {
        ProposalOutcome::Pending(Box::new(row))
    })
}

/// Review snapshot plus the exact complete command the human approved.
/// Never fill omitted fields from the current mutable draft.
pub struct Review<'a> {
    pub expected_payload: &'a Value,
    pub approved_payload: Value,
}
...
pub async fn approve(
    ...
    let gate = gating::gate(action, &approved_payload, &ctx).await?;
    ...
    if gate.verdict == Verdict::Deny
        || action
            .check_permission(&approved_payload, &ctx)
            .await
            .map_err(|_| DbError::Validation("action permission unavailable".into()))?
            == Decision::Deny
    {
        audit_record(
            &txn,
            ...
            run_seq,
            actor,
            "agent.approval_blocked",
            json!({"proposal_id": proposal_id, "gate": gate}),
        )
        .await?;
        txn.commit().await?;
...
async fn pending(
    ...
    actor: &str,
    action: &dyn Action,
) -> Result<proposal::Model, DbError> {
    if actor.trim().is_empty() {
        return Err(DbError::Validation("human actor required".into()));
    }
    let row = proposal::Entity::find_by_id(id)
        ...
        .await?
        .ok_or_else(conflict)?;
    if actor == row.agent_id {
        return Err(DbError::Validation("agent cannot approve itself".into()));
    }
    Ok(row)
}
...
async fn resolve(
    ...
    let changed = proposal::Entity::update_many()
        .set(proposal::ActiveModel {
            status: Set(status.into()),
            verdict_by: Set(Some(actor.into())),
            payload_json: Set(redaction::scrub(&original, action.private_fields()).to_string()),
            preview_json: Set(scrub_stored(&row.preview_json)?),
            edited_payload_json: Set(
                edited.map(|v| redaction::scrub(v, action.private_fields()).to_string())
            ),
    ...
    audit_record(
        ...
        if status == "approved" {
            ...
            "agent.proposal_denied"
        },
        json!({"proposal_id": row.id, "action": action.name(), "gate": gate,
            "edited": edited.is_some_and(|v| v != &original),
            "payload_sha256": edited.map(payload_digest)}),
    )
    .await
}

// Sorted object keys + serde_json compact encoding; array order and every
// value remain significant. This hashes the unredacted reviewed command only.
