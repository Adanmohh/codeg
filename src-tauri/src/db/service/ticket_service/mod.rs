...
pub(crate) async fn require_inbox<C: ConnectionTrait>(
    conn: &C,
    scope: Scope,
) -> Result<inbox::Model, DbError> {
...
pub async fn create_inbox(
    ...
    if account_id <= 0 || name.trim().is_empty() {
        return Err(DbError::Validation(
            "account and inbox name required".into(),
        ));
    }
    let now = Utc::now();
    Ok(inbox::ActiveModel {
        ...
        channel_type: Set("Channel::Email".into()),
        enable_auto_assignment: Set(false),
        created_at: Set(now),
        updated_at: Set(now),
        ..Default::default()
    }
...
pub async fn list_inboxes<C: ConnectionTrait>(
    conn: &C,
    account_id: i32,
) -> Result<Vec<inbox::Model>, DbError> {
    Ok(inbox::Entity::find()
        .filter(inbox::Column::AccountId.eq(account_id))
...
pub async fn get_contact<C: ConnectionTrait>(
    conn: &C,
    scope: Scope,
    conversation_id: i32,
) -> Result<contact::Model, DbError> {
...
async fn claim_writer<C: ConnectionTrait>(conn: &C, scope: Scope) -> Result<(), DbError> {
    ...
    if result.rows_affected() == 0 {
        return Err(DbError::NotFound("ticket inbox in account".into()));
    }
    Ok(())
}
...
fn required_message_id(value: &str) -> Result<&str, DbError> {
    let source_id = threading::message_id(value);
    ...
    {
        return Err(DbError::Validation(
            "decoded message-id required for idempotency".into(),
        ));
    }
    Ok(source_id)
}

fn validate_content(content: &str) -> Result<(), DbError> {
    // Chatwoot Message's character limit, including content containing NUL
    // (SQLite's length(TEXT) stops at NUL, so the DB check alone is insufficient).
    if content.chars().count() > 150_000 {
        return Err(DbError::Validation(
            "message exceeds 150000 characters".into(),
        ));
    }
    Ok(())
}
...
pub async fn ingest_email(
    ...
    scope: Scope,
    mail: IncomingEmail,
) -> Result<IngestedEmail, DbError> {
    let sender_email = normalized_email(&mail.sender_email)?;
    let source_id = required_message_id(&mail.message_id)?;
    ...
    let sender = match contact::Entity::find()
        .filter(contact::Column::AccountId.eq(scope.account_id))
        .filter(contact::Column::Email.eq(&sender_email))
        .one(&txn)
        .await?
    {
        ...
        None => {
            contact::ActiveModel {
                ...
                name: Set(mail
                    .sender_name
                    .clone()
                    .unwrap_or_else(|| sender_email.split('@').next().unwrap_or_default().into())),
                email: Set(sender_email.clone()),
                phone_number: Set(None),
                identifier: Set(None),
                contact_type: Set(0),
                blocked: Set(false),
                additional_attributes: Set("{}".into()),
                custom_attributes: Set("{}".into()),
                created_at: Set(now),
                updated_at: Set(now),
                ..Default::default()
            }
    ...
    let conversation = match existing {
        ...
        None => {
            let link = match contact_inbox::Entity::find()
                ...
                .filter(contact_inbox::Column::ContactId.eq(sender.id))
                .one(&txn)
                .await?
            {
                Some(link) => link,
                None => {
                    contact_inbox::ActiveModel {
                        account_id: Set(scope.account_id),
                        inbox_id: Set(scope.inbox_id),
                        contact_id: Set(sender.id),
                        source_id: Set(sender_email),
                        created_at: Set(now),
                        updated_at: Set(now),
                        ..Default::default()
                    }
            ...
            conversation::ActiveModel {
                ...
                inbox_id: Set(scope.inbox_id),
                contact_id: Set(sender.id),
                contact_inbox_id: Set(link.id),
                uuid: Set(uuid::Uuid::new_v4().to_string()),
                status: Set(if sender.blocked { 1 } else { 0 }),
                priority: Set(None),
                assignee_id: Set(None),
                assignee_agent_bot_id: Set(None),
                team_id: Set(None),
                ...
                waiting_since: Set(Some(now)),
                first_reply_created_at: Set(None),
                created_at: Set(now),
                updated_at: Set(now),
                ..Default::default()
            }
    ...
    let message = message::ActiveModel {
        ...
        content_type: Set(8),
        status: Set(0),
        sender_type: Set("Contact".into()),
        sender_id: Set(sender.id.to_string()),
...
pub async fn assign(
    ...
    active.assignee_id = Set(user);
    active.assignee_agent_bot_id = Set(bot);
...
pub async fn set_status(
    ...
    let (status, snooze) = match status {
        ConversationStatus::Open => (0, None),
        ConversationStatus::Resolved => (1, None),
        ConversationStatus::Pending => (2, None),
        ConversationStatus::Snoozed(until) => (3, Some(until)),
    };
    active.status = Set(status);
    active.snoozed_until = Set(snooze);
    if status == 1 {
...
pub async fn add_private_note(
    ...
    record_authored_message(conn, scope, id, author, content, None).await
}

/// Persist a public human reply whose external message-id is already known.
/// This is a receipt/import operation, not a send queue or an approval API.
/// The Step 2 caller must enforce approval and confirm transport success first.
pub async fn record_public_reply(
    conn: &DatabaseConnection,
    scope: Scope,
    id: i32,
    author: &str,
    ...
    let source_id = required_message_id(message_id)?;
    record_authored_message(conn, scope, id, author, content, Some(source_id)).await
}

async fn record_authored_message(
    conn: &DatabaseConnection,
    scope: Scope,
    id: i32,
    author: &str,
    ...
    if author.trim().is_empty() {
        return Err(DbError::Validation("message author required".into()));
    }
    validate_content(content)?;
    let txn = conn.begin().await?;
    claim_writer(&txn, scope).await?;
    let ticket = get_conversation(&txn, scope, id).await?;
    if let Some(source_id) = source_id {
            ...
            .await?
        {
            if existing.conversation_id != id
                ...
                || existing.sender_id != author
                || existing.message_type != 1
                || existing.private
            {
                return Err(DbError::Conflict(
                    "message-id already belongs to another message".into(),
                ));
            }
            txn.commit().await?;
            return Ok(existing);
    ...
    let first_reply = ticket.first_reply_created_at.is_none();
    let mut conversation = ticket.into_active_model();
    let now = Utc::now();
    let message = message::ActiveModel {
        account_id: Set(scope.account_id),
        inbox_id: Set(scope.inbox_id),
        ...
        message_type: Set(1),
        private: Set(source_id.is_none()),
        content: Set(content.into()),
        content_type: Set(0),
        status: Set(0),
        sender_type: Set("User".into()),
        sender_id: Set(author.into()),
        source_id: Set(source_id.map(str::to_owned)),
        content_attributes: Set("{}".into()),
        additional_attributes: Set("{}".into()),
        created_at: Set(now),
        updated_at: Set(now),
        ..Default::default()
    ...
    .insert(&txn)
    .await?;
    conversation.last_activity_at = Set(now);
    conversation.updated_at = Set(now);
    // A private note does not count as a human reply (Message.valid_first_reply?).
    if source_id.is_some() {
        conversation.waiting_since = Set(None);
        if first_reply {
            conversation.first_reply_created_at = Set(Some(now));
        }
    }
