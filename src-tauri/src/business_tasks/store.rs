...
use super::{agent, entity::{activity, deliverable, execution, task}, policy, types::*, validation::*, vocabulary::*, ActorContext};
use crate::business_identity::{self as identity, IdentityError as E, Member, MemberKind, Permission};
use sea_orm::{ConnectionTrait, DatabaseConnection, DatabaseTransaction, DbBackend, FromQueryResult, Statement, TransactionTrait, Value};
use serde_json::json;
...
fn kind(member: &Member) -> &'static str {
    match member.kind { MemberKind::Human => "human", MemberKind::Agent => "agent" }
}
fn domain(row: &task::Model) -> Result<TaskDomain, E> {
...
fn metadata(title: &str, notes: &str, due_date: Option<&str>) -> Result<(), E> {
    if !valid_text(title, MAX_TITLE, true) || !valid_text(notes, MAX_TEXT, false) {
        return Err(E::Invalid("Use a nonblank title up to 240 characters and notes up to 20000 characters"));
    }
    if !valid_due_date(due_date) { return Err(E::Invalid("Use a valid calendar date YYYY-MM-DD")); }
    Ok(())
}
fn body(text: &str, required: bool) -> Result<(), E> {
    if valid_text(text, MAX_TEXT, required) { Ok(()) }
    else { Err(E::Invalid("Use plain text up to 20000 characters")) }
}
pub(super) async fn model<C: ConnectionTrait>(conn: &C, org: &str, task_id: &str) -> Result<task::Model, E> {
...
async fn visible<C: ConnectionTrait>(conn: &C, ctx: &ActorContext, row: &task::Model, permission: Permission) -> Result<Member, E> {
    let member = identity::authorize(conn, &ctx.principal, &row.organization_id, Permission::Read, Some(domain(row)?)).await
        .map_err(|e| if matches!(e, E::Forbidden) { E::NotFound } else { e })?;
    agent::scope(conn, ctx, row, member.kind).await?;
    if permission != Permission::Read {
        identity::authorize(conn, &ctx.principal, &row.organization_id, permission, Some(domain(row)?)).await?;
    }
    Ok(member)
}
async fn references<C: ConnectionTrait>(conn: &C, row: &task::Model) -> Result<(), E> {
    let d = domain(row)?;
    let owner = identity::active_reference(conn, &row.organization_id, &row.owner_id, d).await?;
    if owner.kind != MemberKind::Human || !owner.allows(Permission::Contribute, Some(d)) { return Err(E::NotFound); }
    if let Some(id) = &row.assignee_id {
        let assignee = identity::active_reference(conn, &row.organization_id, id, d).await?;
        if !assignee.allows(Permission::Contribute, Some(d)) { return Err(E::NotFound); }
    }
    if let Some(id) = &row.reviewer_id {
        let reviewer = identity::active_reference(conn, &row.organization_id, id, d).await?;
        if reviewer.kind != MemberKind::Human || !reviewer.allows(Permission::Review, Some(d)) { return Err(E::NotFound); }
    }
    Ok(())
}
async fn caps<C: ConnectionTrait>(conn: &C, row: &task::Model, member: &Member) -> Result<Capabilities, E> {
    let d = domain(row)?;
    let mut result = policy::capabilities(row, member, d);
    if result.link_execution {
        result.link_execution = match &row.assignee_id {
            Some(id) => identity::active_reference(conn, &row.organization_id, id, d).await
                .is_ok_and(|m| m.kind == MemberKind::Agent && m.allows(Permission::Contribute, Some(d))),
            None => false,
        };
    }
    Ok(result)
}
async fn task_dto<C: ConnectionTrait>(conn: &C, row: task::Model, member: &Member) -> Result<Task, E> {
    let capabilities = caps(conn, &row, member).await?;
    let d = domain(&row)?;
    Ok(Task { id: row.id, organization_id: row.organization_id, title: row.title, notes: row.notes,
        domain: d, status: row.status, priority: row.priority, due_date: row.due_date,
        owner_id: row.owner_id, assignee_id: row.assignee_id, creator_id: row.creator_id,
        reviewer_id: row.reviewer_id, revision: row.revision, current_deliverable_id: row.current_deliverable_id,
        created_at: row.created_at, updated_at: row.updated_at, archived_at: row.archived_at, capabilities })
}
async fn detail_in<C: ConnectionTrait>(conn: &C, ctx: &ActorContext, row: task::Model) -> Result<Detail, E> {
    ...
    let execution = match &row.current_execution_id {
        ...
        Some(id) => {
            ...
            let live = agent::LiveExecution { work_task_id: binding.work_task_id, run_seq: binding.run_seq,
                connection_id: binding.connection_id.clone(), agent_key: binding.agent_key.clone() };
            let active = binding.revoked_at.is_none() && !policy::terminal(row.status)
                && row.archived_at.is_none() && row.assignee_id.as_deref() == Some(binding.agent_member_id.as_str())
                && agent::require_live(conn, &live).await.is_ok()
                && identity::agent_principal_from_binding(conn, &row.organization_id, &binding.agent_member_id, &binding.delegation_json).await.is_ok();
            Some(Execution { work_task_id: binding.work_task_id, run_seq: binding.run_seq, agent_member_id: binding.agent_member_id, active })
        }
    };
    Ok(Detail { task: task_dto(conn, row, &actor).await?, activity, deliverables, execution })
}

pub async fn get(conn: &DatabaseConnection, ctx: &ActorContext, input: TaskInput) -> Result<Detail, E> {
...
pub async fn list(conn: &DatabaseConnection, ctx: &ActorContext, input: ListInput) -> Result<TaskPage, E> {
    if input.page > 100_000 || input.query.as_deref().is_some_and(|q| !valid_text(q, 240, false)) {
        return Err(E::Invalid("Invalid task filter"));
    }
    let tx = conn.begin().await?;
    let org = ctx.principal.organization_id();
    let actor = identity::authorize(&tx, &ctx.principal, org, Permission::Read, input.domain).await?;
    // Agent context has one linked task; no organization-wide list surface.
    if actor.kind != MemberKind::Human || ctx.live.is_some() { return Err(E::Forbidden); }
    let allowed: Vec<_> = TaskDomain::ALL.into_iter().filter(|d| actor.allows(Permission::Read, Some(*d))
        && input.domain.is_none_or(|wanted| wanted == *d)).collect();
    let can_create = allowed.iter().any(|d| actor.allows(Permission::Create, Some(*d)));
    if allowed.is_empty() { return Ok(TaskPage { tasks: vec![], page: input.page, has_more: false, can_create }); }
    ...
    if matches!(input.view, TaskView::Mine) {
        ...
        values.extend([actor.id.clone().into(), actor.id.clone().into(), actor.id.clone().into()]);
    }
    if let Some(s) = input.status { sql.push_str(" AND status = ?"); values.push(status_key(s).into()); }
    if let Some(q) = input.query.filter(|q| !q.trim().is_empty()) {
        sql.push_str(" AND (instr(lower(title), lower(?)) > 0 OR instr(lower(notes), lower(?)) > 0)");
    ...
    let mut rows = task::Model::find_by_statement(statement(&sql, values)).all(&tx).await?;
    let has_more = rows.len() > 50;
    rows.truncate(50);
    let mut tasks = Vec::with_capacity(rows.len());
    for row in rows { tasks.push(task_dto(&tx, row, &actor).await?); }
    tx.commit().await?;
    Ok(TaskPage { tasks, page: input.page, has_more, can_create })
}

async fn audit<C: ConnectionTrait>(conn: &C, row: &task::Model, actor: &Member, action: &str, payload: serde_json::Value) -> Result<(), E> {
...
pub async fn create(conn: &DatabaseConnection, ctx: &ActorContext, input: CreateInput) -> Result<Detail, E> {
    metadata(&input.title, &input.notes, input.due_date.as_deref())?;
    let org = ctx.principal.organization_id();
    let tx = identity::begin_write(conn, org).await?;
    let actor = identity::authorize(&tx, &ctx.principal, org, Permission::Create, Some(input.domain)).await?;
    if actor.kind != MemberKind::Human || ctx.live.is_some() { return Err(E::Forbidden); }
    let owner_id = input.owner_id.unwrap_or_else(|| actor.id.clone());
    if owner_id != actor.id || input.assignee_id.as_deref().is_some_and(|id| id != actor.id) || input.reviewer_id.is_some() {
        identity::authorize(&tx, &ctx.principal, org, Permission::Assign, Some(input.domain)).await?;
    }
    let timestamp = now();
    let row = task::Model { id: id(), organization_id: org.to_owned(), title: input.title, notes: input.notes,
        domain: domain_key(input.domain).to_owned(), status: TaskStatus::Todo, priority: input.priority,
        due_date: input.due_date, owner_id, assignee_id: input.assignee_id, creator_id: actor.id.clone(), reviewer_id: input.reviewer_id,
        revision: 1, current_deliverable_id: None, current_execution_id: None, created_at: timestamp.clone(), updated_at: timestamp, archived_at: None };
...
async fn begin_task(conn: &DatabaseConnection, ctx: &ActorContext, task_id: &str, revision: i64) -> Result<(DatabaseTransaction, task::Model, Member, Capabilities), E> {
    if revision <= 0 { return Err(E::Invalid("expectedRevision must be positive")); }
    ...
    if row.revision != revision { return Err(E::Conflict); }
    let capabilities = caps(&tx, &row, &actor).await?;
    Ok((tx, row, actor, capabilities))
}
fn require(allowed: bool) -> Result<(), E> { if allowed { Ok(()) } else { Err(E::Forbidden) } }
...
fn invalidate_review(row: &mut task::Model) {
    if row.status == TaskStatus::Review { row.status = TaskStatus::InProgress; }
    row.current_deliverable_id = None;
}
async fn finish(tx: DatabaseTransaction, ctx: &ActorContext, mut row: task::Model, actor: &Member, action: &str, payload: serde_json::Value) -> Result<Detail, E> {
    ...
    if changed.rows_affected() != 1 { return Err(E::Conflict); }
...
pub async fn update(conn: &DatabaseConnection, ctx: &ActorContext, input: UpdateInput) -> Result<Detail, E> {
    metadata(&input.title, &input.notes, input.due_date.as_deref())?;
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.edit)?;
    identity::authorize(&tx, &ctx.principal, &row.organization_id, Permission::Contribute, Some(input.domain)).await?;
    let changed_domain = domain(&row)? != input.domain;
    row.title = input.title; row.notes = input.notes; row.domain = domain_key(input.domain).into(); row.priority = input.priority; row.due_date = input.due_date;
    references(&tx, &row).await?;
    if changed_domain { revoke(&tx, &row).await?; }
...
pub async fn assign(conn: &DatabaseConnection, ctx: &ActorContext, input: AssignInput) -> Result<Detail, E> {
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.assign)?;
    row.owner_id = input.owner_id; row.assignee_id = input.assignee_id; row.reviewer_id = input.reviewer_id;
    references(&tx, &row).await?;
    revoke(&tx, &row).await?;
    invalidate_review(&mut row);
...
pub async fn progress(conn: &DatabaseConnection, ctx: &ActorContext, input: ProgressInput) -> Result<Detail, E> {
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.progress)?;
    references(&tx, &row).await?;
    if policy::terminal(row.status) { revoke(&tx, &row).await?; row.current_deliverable_id = None; }
    let previous = row.status;
    row.status = input.status.into();
    finish(tx, ctx, row, &actor, "progressed", json!({"from": previous, "to": input.status})).await
}

pub async fn note(conn: &DatabaseConnection, ctx: &ActorContext, input: TextInput) -> Result<Detail, E> {
    body(&input.body, true)?;
    let (tx, row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.comment)?;
    references(&tx, &row).await?;
    finish(tx, ctx, row, &actor, "note", json!({"body": input.body})).await
}

pub async fn submit(conn: &DatabaseConnection, ctx: &ActorContext, input: TextInput) -> Result<Detail, E> {
    body(&input.body, true)?;
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.submit)?;
    references(&tx, &row).await?;
    ...
    row.current_deliverable_id = Some(deliverable_id.clone()); row.status = TaskStatus::Review;
    finish(tx, ctx, row, &actor, "submitted", json!({"deliverableId": deliverable_id})).await
}

pub async fn review(conn: &DatabaseConnection, ctx: &ActorContext, input: ReviewInput) -> Result<Detail, E> {
    body(&input.comment, false)?;
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.review)?;
    references(&tx, &row).await?;
    row.status = match input.decision { ReviewDecision::Accept => TaskStatus::Done, ReviewDecision::Return => TaskStatus::InProgress };
    ...
    if row.status == TaskStatus::Done { revoke(&tx, &row).await?; }
    finish(tx, ctx, row, &actor, "reviewed", payload).await
}

pub async fn cancel(conn: &DatabaseConnection, ctx: &ActorContext, input: RevisionInput) -> Result<Detail, E> {
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.cancel)?;
...
pub async fn archive(conn: &DatabaseConnection, ctx: &ActorContext, input: ArchiveInput) -> Result<Detail, E> {
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    require(caps.archive)?;
    row.archived_at = input.archived.then(now);
    revoke(&tx, &row).await?;
    finish(tx, ctx, row, &actor, "archived", json!({"archived": input.archived})).await
}

/// Only the engine may supply live fields, under its existing request lock.
pub(crate) async fn link(conn: &DatabaseConnection, ctx: &ActorContext, input: LinkExecutionInput, live: agent::LiveExecution) -> Result<Detail, E> {
    let (tx, mut row, actor, caps) = begin_task(conn, ctx, &input.task_id, input.expected_revision).await?;
    ...
    if occupied.is_some() { return Err(E::Conflict); }
    ...
    invalidate_review(&mut row);
    finish(tx, ctx, row, &actor, "execution_linked", json!({"workTaskId": live.work_task_id, "runSeq": live.run_seq, "agentMemberId": agent_id})).await
}
