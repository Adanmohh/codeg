...
async fn independent_sqlite_connections_race_one_revision_and_one_activity() {
    ...
    let mut a = update_input(&task.task);
    a.title = "First writer".into();
    let mut b = update_input(&task.task);
    b.title = "Second writer".into();
    let (a, b) = tokio::join!(
        store::update(&db.conn, &ctx, a),
        store::update(&other, &ctx, b)
    );
...
async fn task_writer_rechecks_original_credential_after_waiting_for_revocation() {
    ...
    let task = store::create(&db.conn, &cached, create_input())
        .await
        .unwrap();
    let revocation = identity::begin_write(&db.conn, op.organization_id())
        .await
        .unwrap();
    revocation
...
async fn activity_failure_rolls_back_deliverable_and_revision_and_history_cannot_be_rewritten() {
    ...
    let input = TextInput {
        ...
        expected_revision: 1,
        body: "Exact proposed work".into(),
    };
    assert!(matches!(
        store::submit(&db.conn, &ctx, input.clone()).await,
        Err(E::Database(_))
    ));
    assert_eq!(
        count(
            &db.conn,
            "SELECT COUNT(*) AS count FROM business_task_deliverable"
        )
        .await,
        0
    );
    ...
    .await
    .unwrap();
    assert_eq!(unchanged.task.revision, 1);
    assert_eq!(unchanged.task.status, TaskStatus::Todo);
    db.conn
        .execute_unprepared("DROP TRIGGER fixture_task_audit_failure")
        .await
        .unwrap();
    let saved = store::submit(&db.conn, &ctx, input).await.unwrap();
    assert_eq!(saved.task.revision, 2);
    for sql in [
        "UPDATE business_task_activity SET actor_id = 'forged'",
        "DELETE FROM business_task_activity",
        "UPDATE business_task_deliverable SET body = 'forged'",
        "DELETE FROM business_task_deliverable",
        "DELETE FROM business_task",
    ] {
...
async fn task_migration_is_atomic_scoped_and_refuses_to_drop_durable_history() {
    ...
    db.conn
        .execute_unprepared("DROP TABLE business_task_execution")
        .await
        .unwrap();
