...
Link requires a currently assigned permitted agent and an already live engineering execution resolved by existing TaskEngine under its request lock. Backend derives root connection, run sequence and stable agent wire key; none are request fields. Link does not start/instruct a model, expose legacy routes or give members filesystem authority. One immutable business binding per engineering generation prevents silent retargeting: even after revocation, that generation cannot be linked again to another task or grant. A human must start a new engineering generation before linking again. Otherwise a delayed payload could land on a different task whose numeric revision happened to match.
