...
async page => {
  ...
  const token = credentials.sessions.manager.token;
  if (!token || !page.url().startsWith('http://127.0.0.1:4354/business')) throw Error('Wrong synthetic session');
  const metadata = await page.evaluate(async () => (await fetch('/__business_intake_fixture')).json());
  if (!metadata.synthetic || metadata.backendPort !== 4351 || metadata.workspaceBackendPort !== 4351 || !metadata.export.endsWith('unified-1974d97f')) throw Error('Wrong unified export');
