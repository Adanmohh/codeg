...
return raw;
  ...
  const cases = [];
  const originalDraft = 'Synthetic pane draft retained after the contrast correction.';
  for (const locale of ['en','ar']) for (const theme of ['light','dark']) for (const width of [1280,768,390]) {
    ...
    const name='workbench-'+width+'-'+theme+'-'+locale+'-contrast';
  ...
  await page.screenshot({path:'reports/business-intake-ui-evidence/workbench-390-dark-ar-reduced-contrast.png'});
