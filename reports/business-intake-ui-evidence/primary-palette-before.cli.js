async page => {
  ...
  for(const palette of ['blue','violet','neutral']) {
    ...
    for(const theme of ['light','dark']) {
      ...
      for(const state of ['normal','hover','focus']) {
        ...
        if(state==='focus') { await button.focus();await page.keyboard.press('Tab');await page.keyboard.press('Shift+Tab'); }
      ...
      await page.screenshot({path:'reports/business-intake-ui-evidence/primary-'+palette+'-'+theme+'-before.png'});
    }
  }
  if(await page.getByRole('textbox',{name:'Brief',exact:true}).inputValue()!=='Synthetic pane draft for locale, focus and responsive preservation.')throw Error('Lost private draft');
