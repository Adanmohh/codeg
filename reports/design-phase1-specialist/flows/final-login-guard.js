async (page) => {
  ...
  await page.context().route("**/*", async (route) => {
    ...
    if (path !== "api/ops_design_fixture_stats" && /^api\/(set_|save_|ops_|ops_intake_|acp_pi_set_|check_for_updates)/.test(path)) {
      counts.blockedWrites++
      return route.abort()
    }
    return route.continue()
  })
  return "Owned login recheck guarded; no model/provider/configuration actions"
}
