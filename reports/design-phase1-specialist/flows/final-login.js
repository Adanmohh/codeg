async (page) => {
  ...
  // Public synthetic token from the already running owned4327 fixture.
  const origin = "http://127.0.0.1:4327"
  const width = page.viewportSize().width
  const path = `reports/design-phase1-specialist/screenshots/final-login-${width}`
  ...
  await input.fill("ops-design-synthetic-operator")
  ...
  const providerAfter = await stats()
