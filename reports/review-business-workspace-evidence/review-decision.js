...
async (page) => {
 ...
 const accept=dialog.getByRole('button',{name:'Accept work',exact:true})
 const initiallyDisabled=await accept.isDisabled()
 await dialog.getByLabel('Review note',{exact:true}).fill('Synthetic review accepted: clear customer outcome and named responsibility.')
 await dialog.getByLabel('I have reviewed this task and its current deliverable.',{exact:true}).check()
 await page.keyboard.press('Tab')
 await page.evaluate(()=>new Promise(resolve=>setTimeout(resolve,500)))
 const focus=await page.evaluate(()=>{const e=document.activeElement,s=getComputedStyle(e);return{text:e.textContent,outline:s.outline,border:s.borderColor,borderWidth:s.borderWidth,shadow:s.boxShadow,focusVisible:e.matches(':focus-visible')}})
 if(focus.text!=='Accept work')throw Error('Expected keyboard review focus')
 await page.screenshot({path:'reports/review-business-workspace-evidence/review-focused-390-dark.png'})
 ...
 await dialog.getByText('Revision 6',{exact:true}).waitFor()
 await page.screenshot({path:'reports/review-business-workspace-evidence/review-done-390-dark.png'})
 const result={initiallyDisabled,focus,raw,status:response.status(),submitted:response.request().postDataJSON(),task:detail.task,execution:detail.execution,activity:detail.activity.at(-1),snapshot:await dialog.ariaSnapshot()}
