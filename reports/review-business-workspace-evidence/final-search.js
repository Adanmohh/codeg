// Report-only targeted correction recheck. Reuses accepted capture.js; see NOTICE.md.
async page => {
...
async (page) => {
  ...
  const cases = [
    ...
    {width:390,locale:'ar',theme:'light'},
    {width:390,locale:'ar',theme:'dark'},
  ]
  const results=[]
  for(const c of cases) {
    await page.setViewportSize({width:1280,height:900})
    await page.getByRole('combobox',{name:/^(Language|اللغة)$/}).selectOption(c.locale)
    await page.getByRole('combobox',{name:/^(Appearance|المظهر)$/}).selectOption(c.theme)
    await page.getByRole('heading',{name:c.locale==='en'?'My work':'عملي',exact:true}).waitFor()
    ...
    const search=page.locator('input[maxlength="240"]')
    await search.fill('')
    await search.click()
    await page.keyboard.press('Tab')
    await page.waitForTimeout(500)
    const raw=await capture(page)
    const placeholder=raw.samples.find(s=>s.tag==='INPUT'&&s.kind==='placeholder')
    const focus=raw.focus
    if(!placeholder||!placeholder.measurable||placeholder.disabled) throw Error('Search sample not measurable')
    const focusedRefresh=await page.getByRole('button',{name:c.locale==='en'?'Refresh':'تحديث',exact:true}).evaluate(el=>el===document.activeElement)
    if(!focusedRefresh||focus.box.width<20||focus.box.height<20) throw Error('Tab did not reach visible Refresh')
    const screenshot='final-search-'+c.width+'-'+c.theme+'-'+c.locale+'.png'
    await page.screenshot({path:'reports/review-business-workspace-evidence/'+screenshot})
    const responsePromise=page.waitForResponse(r=>r.url().endsWith('/api/business/tasks/list')&&r.request().postDataJSON()?.input?.query==='Synthetic review')
    await search.fill('  Synthetic review  ')
    await search.press('Enter')
    const response=await responsePromise
    if(response.status()!==200) throw Error('Enter search status '+response.status())
    const request=response.request().postDataJSON()
    const body=await response.json()
    const clearResponse=page.waitForResponse(r=>r.url().endsWith('/api/business/tasks/list')&&!r.request().postDataJSON()?.input?.query)
    await search.fill('')
    await search.press('Enter')
    if((await clearResponse).status()!==200) throw Error('Search clear failed')
    results.push({...c,placeholder,focus,focusedRefresh,documentWidth:raw.documentWidth,enter:{status:response.status(),input:request.input,count:body.tasks.length},screenshot})
  }
  await page.setViewportSize({width:1280,height:900})
  ...
  return {source:'e72cc44b612068e67a3e6dc3bc593f10988ae7ed',artifactSha256:'c029f99d5c92e677c97f2ebe45f4bd185746b73e84e77912a683e1d92de55a54',results,ledger:page.__businessReview}
}
