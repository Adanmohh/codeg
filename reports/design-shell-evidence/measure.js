async (page) => page.evaluate(() => {
  ...
  const statusBar = document.querySelector(".ws-surface-muted.border-t")
  const texts = [...document.querySelectorAll("body *")].filter((el) => visible(el) && [...el.childNodes].some((n) => n.nodeType === Node.TEXT_NODE && n.textContent.trim().length > 2))
  // Stats are empty in this fresh DB. Measure the real inherited foreground
  // for StatusBarStats without injecting records or modifying the DOM.
  if (statusBar && visible(statusBar)) texts.push(statusBar)
  const samples = texts.map((el) => {
    const s = getComputedStyle(el)
    const chain = []
    ...
    return {
      text: el === statusBar ? "[status inherited text]" : el.textContent.trim().slice(0, 450), tag: el.tagName, class: el.className,
