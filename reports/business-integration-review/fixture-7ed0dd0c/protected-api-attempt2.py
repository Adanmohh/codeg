...
def request(label, path, data=None, role="owner", expected=200, method="POST", origin=None):
    ...
    try:
        ...
        item = {"label": label, "path": path, "role": role, "method": method, "status": response.status, "expected": expected, "seconds": round(time.monotonic() - start, 4), "cacheControl": response.getheader("Cache-Control")}
    finally:
        conn.close()
    ...
    # The original-token platform middleware returns a bare401, no JSON body.
