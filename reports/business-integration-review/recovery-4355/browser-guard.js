async page => {
  ...
  page.on('response', response => {
    const path = response.url().split('http://127.0.0.1:4355')[1]?.split('?')[0] || 'nonfixture'
  ...
  await page.context().route('**/*', route => {
    ...
    const local = address.startsWith('http://127.0.0.1:4355/')
    const path = local ? address.slice('http://127.0.0.1:4355'.length).split('?')[0] : 'nonfixture'
  ...
  await page.goto('http://127.0.0.1:4355/business')
