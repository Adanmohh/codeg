...
describe("independent source-entry draft ownership", () => {
  it("guards a task source link before replacing another review's unsaved draft", async () => {
    ...
    // The real modal hides the background from accessible role queries. Prove
    // the original editor remains mounted, then check it after explicit stay.
    expect(brief).toBeInTheDocument()
    expect(brief).toHaveValue("Synthetic unsaved source A sentinel")
    fireEvent.click(screen.getByRole("button", { name: "Keep editing" }))
    expect(await screen.findByRole("textbox", { name: "Brief" })).toHaveValue("Synthetic unsaved source A sentinel")
