...
| --- | --- |
| BI-1 Source setup | Actual protected operator can establish the approved binding, named human audience and publication destinations. Ordinary owner/member credentials cannot configure provider scope or obtain secrets. Missing setup has a usable, honest path. Credential-store/SQLite failure boundaries and rebind fencing are tested explicitly. |
...
| BI-7 Changed source/access | Source refresh retains drafts but requires explicit current-passage rebase. Revoked credentials, grants and binding epochs fence in-flight results and decisions. A→B→A is a new revision, not a stale-pass loophole. |
