// Root rerun of PR18 actual browser measurement on owned synthetic fixture.
...
async (page) => {
  if (!page.url().startsWith("http://127.0.0.1:4326/")) throw Error("Fixture only")
  ...
  if (themeSetting && themeSetting !== "system") throw Error("Requires existing Follow system preference")
  ...
  for (const theme of ["light", "dark"]) {
    ...
    for (const state of ["rest", "hover", "focus"]) {
      ...
      await page.screenshot({ path: `reports/browser-phase1-final/badge-${surface === "group" ? "group-" : ""}${phase}-${theme}-${state}.png` })
    }
  }
  await header.evaluate((el) => el.blur())
